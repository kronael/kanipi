...
## Operational gaps (marinade)

### Codebase refresh

Marinade repos are symlinked from `eliza_atlas_marinade/codebase/`.
No mechanism to pull updates. Two approaches:

1. **Scheduled task**: cron job runs `git pull` in each repo dir.
   Task scheduler exists, needs a configured task.
2. **MCP service**: `git-clone` tool that agents can invoke to
   refresh or add repos on demand.

Either or both. Task for baseline freshness, MCP for on-demand.
