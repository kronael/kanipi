...
import { DATA_DIR, getMaxConcurrent } from './config.js';
...
export class GroupQueue {
  ...
  private scheduleRetry(groupJid: string, state: GroupState): void {
    ...
    if (state.retryCount > MAX_RETRIES) {
      logger.error({ groupJid, retryCount: state.retryCount }, 'Max retries exceeded');
      state.retryCount = 0;
      return;
    }
  ...
  private drainWaiting(): void {
    const maxConcurrent = getMaxConcurrent();
    while (this.waitingGroups.length > 0 && this.activeCount < maxConcurrent) {
  ...
  async shutdown(_gracePeriodMs: number): Promise<void> {
    ...
    logger.info(
      ...
      'GroupQueue shutting down (containers detached)',
    );
  }
}
