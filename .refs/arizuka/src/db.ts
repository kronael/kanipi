...
function createSchema(database: Database.Database): void {
  database.exec(`
    ...
    -- Arizuka: credential store (Phase 2 CRUD via IPC)
...
/**
 * Get chat JIDs that have messages after lastTimestamp but are NOT in the
 * registeredJids set. Used for auto-registration of unknown chats.
 */
export function getUnregisteredChatsWithMessages(
  ...
  return rows
    .map((r) => r.chat_jid)
    .filter((jid) => !registeredJids.has(jid) && jid !== '__group_sync__');
}
...
export function getRegisteredGroup(jid: string): (RegisteredGroup & { jid: string }) | undefined {
  const row = db.prepare('SELECT * FROM registered_groups WHERE jid = ?').get(jid) as {
    ...
    added_at: string; container_config: string | null; requires_trigger: number | null;
  } | undefined;
  ...
  return {
    jid: row.jid, name: row.name, folder: row.folder, trigger: row.trigger_pattern,
